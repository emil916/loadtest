 ...
 *	- poisson_burstiness: if true, use Poisson distribution for request intervals (requires requestsPerSecond).
...
class Operation {
	...
	startClients() {
		...
		for (let index = 0; index < this.options.concurrency; index++) {
			...
			if (this.options.poisson_burstiness || !this.options.requestsPerSecond) {
				// start all clients immediately if using a Poisson distribution or when not using a fixed rps
				client.start();
			} else {
