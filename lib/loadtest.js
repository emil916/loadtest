 ...
 *	- poisson: if true, use Poisson distribution for request intervals (requires requestsPerSecond).
...
class Operation {
	...
	stop() {
		...
		if (this.completedRequests < this.requests) {
			log.debug('Waiting for all the initiated requests to finish...');
			this.stopTimeout = setTimeout(() => this.stop(), 100).unref();
			return;
		}
