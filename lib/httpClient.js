...
class HttpClient {
	...
	start() {
		...
		if (this.params.poisson) {
			log.debug('Using Poisson distribution for request intervals with lambda = %s rps', rps);
			// Use a different seed for each client to avoid synchronization
			const cpg = new PoissonGenerator('seed-' + client_id++);
			intervalGenerator = cpg.getPoissonInterval.bind(cpg);
			...
			this.requestTimer = new HighResolutionTimer(1000, () => {
				...
				for (var i = 0; i < rps; i++) {
					interval = cpg.getPoissonInterval(rps);
					log.debug('%dth request in %d ms', i, interval);
