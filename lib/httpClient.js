...
class HttpClient {
	...
	start() {
		...
		if (this.params.poisson) {
			log.debug('Using Poisson distribution for request intervals with lambda = %s rps', this.params.requestsPerSecond);
			intervalGenerator = poisson.createPoissonGenerator(this.params.requestsPerSecond);
		}
		
		this.requestTimer = new HighResolutionTimer(interval, () => {
			if (intervalGenerator) {
				// Update interval dynamically for next request
				const nextInterval = intervalGenerator();
				this.requestTimer.delayMs = nextInterval;
			}
			this.makeRequest();
		});
	}
