...
const {PoissonGenerator} = require('./poisson.js');
...
class HttpClient {
	...
	start() {
		...
		let interval = 1000 / this.params.requestsPerSecond;
		const rps = Math.round(this.params.requestsPerSecond);
		
		if (this.params.poisson_burstiness) {
			log.info('Using Poisson distribution for request intervals with lambda = %s rps and burstiness = %s', rps, this.params.poisson_burstiness);
			// Use a different seed for each client to avoid synchronization
			const cpg = new PoissonGenerator('seed-' + client_id);


			this.requestTimer = new HighResolutionTimer(1000, () => {
				// Replace the fixed interval by exponential random interval that follows the Poisson distribution
				var cumulative_time = 0;
				for (var i = 0; i < rps; i++) {
					interval = cpg.getPoissonInterval(rps, this.params.poisson_burstiness);
					log.debug('Client %d: %dth request in %d ms', client_id, i, interval);
	...
	makeRequest() {
		if (!this.operation.running) {
			// log.info('Operation is not running, skipping request');
			return;
		}
		...
		if (this.generateMessage) {
			...
			if (typeof message === 'object' && !this.params.postFileBinary && !this.params.postFileRandom) {
				/* special case when the postFile is used with .js file (?) */
